...
from routes import slicer,slicer_aws,slicer_gcp
...
init(
    ...
    framework='fastapi',
    recipe_list=[session.init(cookie_domain = read_yaml.cookie_domain),jwt_recipe.init()],
    mode='asgi' 
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        read_yaml.website_domain
    ],
    allow_credentials=True,
    allow_methods=["GET", "PUT", "POST", "DELETE", "OPTIONS", "PATCH"],
    allow_headers=["Content-Type","X-ik-authCookie"] + get_all_cors_headers(),
)
...
async def session_middleware(request: Request, call_next):
    ...
    request.state.session_id = session.session_handle
...
async def authenticate_request(request:Request):
    ...
    try:
        ...
        request.state.session_id = payload.get("sessionId",None)
...
app.include_router(slicer_gcp.router)
