...
from state_machine import campaign as sc
from campaign_db.db_client import client

from scheduler.scheduler import site, scheduler
...
async def authenticate_request(request: Request):
    ...
    jwt_token = utils.get_token_from_request_headers(request)
    ...
    rsa_key = await utils.get_key(header)

    try:
        payload = jwt.decode(jwt_token, rsa_key, algorithms=["RS256"])
        print("Received jwt payload", payload)
...
app.include_router(campaign_tier_new.router)
