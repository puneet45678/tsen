...
class SumoLogicHandler:
    def __init__(self, url, batch_size=25):
        self.url = url
...
def get_db_action_Logger(name):
    log_format = (
       "<blue>{time:YYYY-MM-DD HH:mm:ss.SSS}</blue> | <level>{level}</level> | {name} | <green>{function}</green> | <magenta>{file}:{line} </magenta>| <cyan>{message}</cyan>"

    )

    log_file = os.path.join(directory, "campaign.log")
    url = "https://collectors.in.sumologic.com/receiver/v1/http/ZaVnC4dhaV2beGZOdaKZ8ZCcvPzCowIYnCmXOn-NHzRxYtwa-MMjSqVwaBUhlA6tC0r7Oh1LTpOrFARath6ShCqIiiSraYO-UnDYJ4DHEvvXRAdflxcUug=="
