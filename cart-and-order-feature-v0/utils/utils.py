...
async def create_jwt_and_auth_headers(userid , session= None, session_payload = None):
...
def create_item_lists_object(items):
    payload = {}
    for item in items:
        if item["itemType"] not in payload:
            payload[item["itemType"]] = []
        payload[item["itemType"]].append(item["itemId"])
    return payload
