...
export default function YourCampaignPublished({your_campaign_published_activities}) {
    const [profilePicture, setProfilePicture] = useState("");
  const user_current = useSelector((state) => state.user);
  let latest_campaign_published_activities = your_campaign_published_activities;
            ...
            <div className="relative rounded-full w-10 h-10 border-[1px] border-icons-border bg-accent1">
                ...
                <div className="relative w-[48px] h-[48px]">
                  <Image
                    ...
                    alt="Profile Picture"
                    src={your_campaign_published_activities?.text?.actor_profile_picture_url}
                    fill
                  />
                </div>
              ...
              <div className="flex justify-between">
                <span className="mt-[10px] text-black text-[15px]">
                  <div className="hover:cursor-pointer flex gap-1 justify-between">
                    <p className="">
                   <span className="text-sm font-[400]">Congratulations! Your Campaign </span><span className="text-lg text-primary-purple-500 font-[600] hover:underline">{your_campaign_published_activities?.text?.campaign_name}</span><span className="text-sm font-[400]"> is published</span>
                    </p>
              ...
              <div className="flex gap-[24px] mt-[4%] w-full items-end">
                <div className="relative w-[100px] h-[100px] border-[1px] border-icons-border bg-accent1">
                      ...
                      <Image
                        ...
                        alt="Portfolio Cover"
                        src={your_campaign_published_activities?.text?.cover_image_url}
                        fill
                      />
                      l{" "}
                    
                </div>
                <button onClick={()=>{}} className="mt-0 px-[12px] py-[6px] border-[1px] h-fit border-primary-purple-600 text-primary-purple-500 rounded-[4px]">View on marketplace</button>
              </div>
            </div>
          </div>
