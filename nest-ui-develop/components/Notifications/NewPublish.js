...
export default function MarketingNotification({ new_publish_activities }) {
  // const { user } = useStreamContext();
  // console.log("MarketingUser: ",user);
  const [profilePicture, setProfilePicture] = useState("");
  const user_current = useSelector((state) => state.user);
  let latest_new_publish_activities = new_publish_activities;
  // console.log("market_activities: ",marketingActivities)
  let timeStamp = latest_new_publish_activities?.time;
              ...
              <div className="flex gap-4 mt-[4%]">
                ...
                <div className="flex flex-col gap-4 justify-between">
                  <p className="flex gap-2 text-[#888] mt-10px ">
                    ...
                    {latest_new_publish_activities!==undefined?parse(latest_new_publish_activities?.text):null}
                  </p>
