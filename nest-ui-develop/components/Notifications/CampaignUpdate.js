...
export default function CommentNotification({ campaign_update_activities }) {
  // const { user } = useStreamContext();

  const [profilePicture, setProfilePicture] = useState("");
  const user_current = useSelector((state) => state.user);
//   console.log("CommentActivity: ", campaign_update_activities);
  let campaign_update_activities = campaign_update_activities;
  // let name = commentActivitiesLatest.actor.data.name;
  let timeStamp = campaign_update_activities.time;
  useEffect(() => {
    setProfilePicture(
  ...
  return (
    <>
      {/* {pre_market_signup_activities.map((cAct) => { */}
              ...
              <div className="flex justify-between">
                <span className="mt-[10px] text-black text-[15px]">
                  <div className="hover:cursor-pointer flex gap-1 justify-between">
                    <div className="hover:underline font-semibold">{name}</div>
                    <div>Posted a new update on campaign</div>
                  </div>
                </span>
              ...
              <div className="flex gap-4 mt-[4%]">
                ...
                <div className="flex flex-col gap-4 justify-between">
                  <p className="flex gap-2 text-[#888] mt-10px ">
                    ...
                    {campaign_update_activities!==undefined?parse(campaign_update_activities?.text):null}
                  </p>
